...
{
    private Mock<IAiPeopleInformationFinder> _aiMock;
    private Mock<ISearchRequestDetailsRepository> _repoMock;
    private PeopleFinderService _sut;
    ...
    [Test]
    public async Task GetSearchTermSuggestions_CallsRepositoryGetSearchTermSuggestions()
    {
        // arrange
        _repoMock.Setup(r => r.GetSearchTermSuggestions("searchTerm", 10)).ReturnsAsync(["item1"]);
        
        // act
        var result = await _sut.GetSearchTermSuggestions("searchTerm", 10);
        
        // assert
        Assert.That(result, Is.EquivalentTo(["item1"]));
        _repoMock.Verify(r => r.GetSearchTermSuggestions("searchTerm", 10), Times.Once);
    }
}